...
export default function ProductPage() {
	...
	useEffect(() => {
		...
		if (cookies.mini === "true") {
			...
			document.cookie =
				"mini=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
		}
	}, []);
				...
				<div className="grid md:grid-cols-2 gap-12 items-start">
					...
					<div className="space-y-6">
							...
							<div className="flex items-center gap-2">
								<div className="flex items-center gap-1">
									{Array.from({ length: 5 }).map((_, i) => (
										<Star
											key={i}
											className={`h-5 w-5 ${i < product.rating ? "fill-coral text-coral" : "fill-none text-charcoal/20"
												}`}
										/>
									))}
								</div>
						...
						<div className="space-y-3 pt-4">
							<a href={product.amazonUrl} data-auto className="block">
								<Button
									size="lg"
